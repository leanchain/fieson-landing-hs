...
const MarkdownPage: React.FC<{ directory?: string; filename?: string }> = ({
  directory: propDirectory,
  filename: propFilename,
}) => {
  const { directory: paramDirectory, filename: paramFilename } = useParams<{
    directory?: string;
    filename: string;
  }>();
    ...
    const fetchMarkdown = async () => {
      try {
        ...
        if (!response.ok) {
          ...
          console.error(
            `Failed to fetch ${filename}.md: ${response.statusText}`,
            errorText
          );
          throw new Error(
            `Failed to fetch ${filename}.md: ${response.statusText}. Details: ${errorText}`
          );
        }
